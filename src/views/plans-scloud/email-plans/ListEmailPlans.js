...
export default function ListEmailPlans() {
  ...
  useEffect(() => {
    if (dataRoles.length > 0) {
      const filteredAdd = dataRoles.filter((role_add) => role_add.function_id === '66746678f7f723b779b1b065');
      const filteredUpdate = dataRoles.filter((role_update) => role_update.function_id === '66746678f7f723b779b1b066');
      const filteredDelete = dataRoles.filter((role_delete) => role_delete.function_id === '66746678f7f723b779b1b067');
  ...
  const handleConfirmDelete = () => {
    if (selectedId) {
      apiDelete(`${LIST_EMAIL_PLANS}`, selectedId)
        .then(() => {
          setOpen(true);
